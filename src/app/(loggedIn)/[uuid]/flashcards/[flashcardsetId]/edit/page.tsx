...
import {loadFlashcardSet, saveFlashcardsBulk, saveFlashcardSet} from "@/lib/server-actions/flashcards";
...
export default function Edit({ params }: PageProps) {
	...
	useEffect(() => {
		const fetchData = async () => {
			setLoading(true);
			try {
				const data = await loadFlashcardSet(flashcardsetId, uuid);
	...
	const handleSave = async () => {
		...
		setSaving(true);
		try {
			const setRes = await saveFlashcardSet(flashcardsetId, { title, description });

			// 2. Save Cards
			const cardsRes = await saveFlashcardsBulk(validFlashcards, idsToDelete);

			if (setRes.success && cardsRes.success) {
				toast.success("Everything saved!");
			} else {
				toast.error(setRes.error || "Failed to save flashcards");
			}
		} catch (error) {
	...
	return (
		<div className="min-h-screen bg-background p-4 md:p-6">
			<div className="mx-auto space-y-6">
				{/* Header - Fixed constraints for mobile */}
				<div
					className="sticky top-0 z-20 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 md:px-6 py-4">
					<div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
						<Button
							variant="ghost"
							size="sm"
							onClick={() => router.push(`/${uuid}/flashcards/${flashcardsetId}`)}
						...
						{flashcards.map((card, index) => (
							<Card key={card.id} className="p-4 md:p-6 relative">
								<div className="flex flex-col md:flex-row items-start gap-4">
									{/* Card Number and Delete Row for Mobile */}
									<div className="flex items-center justify-between w-full md:w-auto">
										<div
											className="w-8 h-8 rounded-full bg-popover flex items-center justify-center font-semibold text-sm shrink-0">
											{index + 1}
										</div>
										<Button
											variant="ghost"
