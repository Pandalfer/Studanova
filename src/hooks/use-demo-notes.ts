...
export function useDemoNotes() {
  ...
  const placeNoteInTree = (
    ...
    if (!note.folderId) {
      ...
      let newRootNotes;
      if (exists) {
        newRootNotes = currentRootNotes.map((n) =>
          n.id === note.id ? note : n,
        );
      } else {
    ...
    const recursiveUpdate = (list: Folder[]): Folder[] => {
      return list.map((f) => {
        if (f.id === note.folderId) {
          ...
          return {
            ...f,
            notes: newNotes.sort((a, b) => a.title.localeCompare(b.title)),
          };
        }
    ...
    return {
      newRootNotes: currentRootNotes,
      newFolders: recursiveUpdate(currentFolders),
    };
  };
  ...
  const saveCurrentNote = useCallback(() => {
    ...
    setRootNotes((prevRoot) => {
      setFolders((prevFolders) => {
        const existingNote = findNoteById(
    ...
    const { newRootNotes: tempRoot, newFolders: tempFolders } =
      removeNoteFromTree(noteToSave.id, rootNotes, folders);
    const { newRootNotes, newFolders } = placeNoteInTree(
      noteToSave,
      tempRoot,
      tempFolders,
  ...
  const deleteNote = (id: string) => {
    const { newRootNotes, newFolders } = removeNoteFromTree(
      id,
      rootNotes,
      folders,
    );
  ...
  const renameNote = (note: Note, newTitle: string) => {
    ...
    const { newRootNotes: tempRoot, newFolders: tempFolders } =
      removeNoteFromTree(note.id, rootNotes, folders);
    const { newRootNotes, newFolders } = placeNoteInTree(
      updatedNote,
      tempRoot,
      tempFolders,
  ...
  const moveNoteToFolder = (noteId: string, targetFolderId?: string) => {
    ...
    const { newRootNotes: tempRoot, newFolders: tempFolders } =
      removeNoteFromTree(noteId, rootNotes, folders);
  ...
  useEffect(() => {
    ...
    const cleanRootNotes = loadedNotes.filter(
      (n) => !allFolderNoteIds.has(n.id),
    );
  ...
  const deleteFolder = (folderId: string) => {
    const isNoteInFolder = (fId: string, notesToCheck: Note[]): boolean => {
      if (notesToCheck.some((n) => n.id === activeNoteIdRef.current))
        return true;
      return false;
    };
  ...
  const moveFolderToFolder = (folderId: string, targetParentId?: string) => {
    ...
    const isDescendant = (parent: Folder, targetId: string): boolean => {
      return (parent.folders || []).some(
        (f) => f.id === targetId || isDescendant(f, targetId),
      );
    };
