...
import {createFlashcardSet, generateFlashcardsFromNote, loadFlashcardSets} from "@/lib/server-actions/flashcards";
...
import {createFlashcardSetDemo, loadFlashcardSetsDemo} from "@/lib/flashcards/flashcard-actions";
...
export function FlashcardsManager({ uuid, isDemo }: FlashcardsContentProps) {
	...
	// 1. Initialize Search Worker
	useEffect(() => {
		workerRef.current = new Worker(new URL("@/lib/flashcards/flashcard-search-worker.ts", import.meta.url));
	...
	useEffect(() => {
		const loadData = async () => {
			...
			try {
				...
				// Load Notes (Branching for Demo)
				const loadedNotes = isDemo ? loadDemoNotes() : await loadNotes(uuid);
				const loadedFolders = isDemo ? loadDemoFolders() : await loadFolders(uuid);
	...
	const handleCreateManual = async () => {
		...
		const response = isDemo
			? await createFlashcardSetDemo(uuid, { title: "New Set", description: "" })
		...
		toast.promise(
			(async () => {
				const res = await generateFlashcardsFromNote({
					uuid, // Server action will check Upstash using this ID
		...
		<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
			...
			<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
				...
				<div className="flex flex-wrap items-center gap-3">
					<Button variant="outline" className="flex-1 md:flex-none" onClick={handleCreateManual}>
					...
					<Dialog modal={false} open={aiGenerateOpen} onOpenChange={setAiGenerateOpen}>
						...
						<DialogContent>
							...
							{isDemo && <p className="text-xs text-muted-foreground">Demo users: 3 generations per month.</p>}
							<form onSubmit={handleAiGenerate} className="space-y-6 pt-4">
								<div className="space-y-2">
									<Label>Source Note</Label>
									<NoteSearcher setSelectedNote={setNote} notes={allNotes} isLoading={isLoadingNotes} />
								</div>
								<div className="space-y-2">
									<Label htmlFor="count">Number of flashcards</Label>
									<Input id="count" name="count" type="number" min={1} max={50} required defaultValue={5} />
								</div>
								<div className="flex justify-end gap-3 pt-2">
									<Button type="button" variant="ghost" onClick={() => setAiGenerateOpen(false)}>Cancel</Button>
									<Button type="submit" disabled={!note}>Generate</Button>
								</div>
							</form>
						</DialogContent>
					</Dialog>
			...
			<div className="mb-10 w-full">
				<InputGroup className="rounded-full h-12 text-base">
					...
					<InputGroupAddon className="pl-4 pr-1">
						{searchQuery ? <X className="h-5 w-5 cursor-pointer" onClick={() => setSearchQuery("")} /> : <Search className="h-5 w-5 text-muted-foreground" />}
					</InputGroupAddon>
				</InputGroup>
			</div>
			...
			<div className="flex flex-col gap-3">
				{isLoadingSets ? (
					...
					filteredSets.map((set) => <FlashcardSetComponent key={set.id} uuid={uuid} {...set} />)
				) : (
					<FlashcardManagerEmptyState onAiClick={() => setAiGenerateOpen(true)} hasQuery={!!searchQuery} />
				)}
			</div>
		</div>
	);
...
function FlashcardManagerEmptyState({ onAiClick, hasQuery }: { onAiClick: () => void; hasQuery: boolean }) {
	return (
		<div className="w-full py-24 flex flex-col items-center justify-center text-center">
			<p className="text-lg text-foreground pb-2">{hasQuery ? "No sets match your search" : "No flashcard sets"}</p>
			{!hasQuery && (
				<>
					<p className="text-muted-foreground mb-6">Get started by creating a set manually or using AI.</p>
					<Button onClick={onAiClick}><Plus className="mr-2 h-4 w-4" /> Create First Set</Button>
				</>
			)}
		</div>
	);
